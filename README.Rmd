...
anes <- anes %>%
  ...
  mutate(educ = factor(educ, level = 1:5, labels = c("HS Not Completed",
                                                     ...
                                                     "Advanced Degree"))) %>% 
  mutate_at(vars(-starts_with("therm"), vote, educ, gender), ~as_factor(.)) %>% 
...
Note that all of the questions with the exception of the actual vote were taken from the pre survey.  If modeling the vote, use the `post_weight` to account for attrition, otherwise use `pre_weight`.  